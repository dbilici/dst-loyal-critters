...
local FailIfRunningDecorator = require("behaviours/failifrunningdecorator")
...
local function FindPlaymate(self)
    ...
    if self.playfultarget ~= nil
        and self.playfultarget:IsValid()
        and can_play
        and TargetCanPlay(self, self.playfultarget, owner, max_dist_from_owner, is_flier) then
        return true
    end

    local find_dist = is_playful and MAX_DOMINANTTRAIT_PLAYFUL_FIND_DIST or MAX_PLAYFUL_FIND_DIST
    self.playfultarget = can_play
        and not owner.components.locomotor:WantsToMoveForward()
        and FindEntity(self.inst, find_dist, function(v)
...
local function ShouldAvoidTarget(self, target)
    ...
    if owner_combat == nil or target_combat == nil then
        return false
    elseif target_combat:TargetIs(owner) or (target.components.grouptargeter ~= nil and target.components.grouptargeter:IsTargeting(owner)) then
        return true
    end
    ...
    local t = GetTime()
    return ((owner_combat:IsRecentTarget(target) or target_combat:HasTarget())
            and math.max(owner_combat.laststartattacktime or 0, owner_combat.lastdoattacktime or 0) + COMBAT_TIMEOUT > t)
        or (owner_combat.lastattacker == target and owner_combat:GetLastAttackedTime() + COMBAT_TIMEOUT > t)
end

local function CombatAvoidanceFindEntityCheck(self)
    return function(ent)
        if ShouldAvoidTarget(self, ent) then
...
local function ValidateCombatAvoidance(self)
    ...
    if not ShouldAvoidTarget(self, self.runawayfrom) then
        self.inst:PushEvent("critter_avoidcombat", { avoid = false })
        self.runawayfrom = nil
        return false
    end
...
local function WatchingMinigame(inst)
    return (inst.components.follower.leader ~= nil
        ...
        and inst.components.follower.leader.components.minigame_participator:GetMinigame()
        or nil
end

local function WatchingMinigameMinDist(inst)
    local minigame = WatchingMinigame(inst)
    return minigame ~= nil and minigame.components.minigame.watchdist_min or 0
end

local function WatchingMinigameTargetDist(inst)
    local minigame = WatchingMinigame(inst)
    return minigame ~= nil and minigame.components.minigame.watchdist_target or 0
end

local function WatchingMinigameMaxDist(inst)
    local minigame = WatchingMinigame(inst)
...
local BetterPetCritterBrain = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
end)

function BetterPetCritterBrain:OnStart()
    local watch_game = WhileNode(function()
            return WatchingMinigame(self.inst)
        end, "Watching Game", PriorityNode({
            Follow(self.inst, WatchingMinigame, WatchingMinigameMinDist, WatchingMinigameTargetDist, WatchingMinigameMaxDist),
    ...
    local root = PriorityNode({
        WhileNode(function()
                ...
                PriorityNode({
                    ...
                    WhileNode(function()
                            return ValidateCombatAvoidance(self)
                        end, "Is Near Combat", FaceEntity(self.inst, GetOwner, KeepFaceTargetFn)),
                }),
                WhileNode(function()
                        return FindPlaymate(self)
                    end, "Playful", SequenceNode({
                        WaitNode(6),
                        PriorityNode({
                            Leash(self.inst, function()
                                return self.playfultarget:GetPosition()
                            end, PLAYFUL_OFFSET, PLAYFUL_OFFSET),
                            ActionNode(function()
                ...
                WhileNode(function()
                        return OwnerIsClose(self.inst) and self.inst:IsAffectionate()
                    end, "Affection", SequenceNode({
                        WaitNode(4),
